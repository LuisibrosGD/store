...
Feature: Acceder a endpoints protegidos con token
  ...
  Scenario Outline: Un usuario accede a un endpoint con un token <tipo>
    Given el usuario tiene un token JWT <tipo>
    When el usuario envía una solicitud a un endpoint protegido con el token
    Then el sistema rechaza la operación con estado 401
         y el mensaje indica que <mensaje>

    Examples:
      | tipo      | mensaje                  |
      | expirado  | el token ha expirado     |
      | inválido  | el token es inválido     |
