...
```gherkin
Feature: Registro público de usuarios

  Scenario: Un visitante registra su cuenta con rol CLIENTE
    Given el email "nuevo@email.com" no está registrado
    When un visitante envía una solicitud para crear un usuario con:
      | nombre       | email             | contraseña | rol     |
      | María García | nuevo@email.com   | pass123    | CLIENTE |
    Then el sistema confirma la creación del usuario
         y el usuario tiene rol CLIENTE
         y puede iniciar sesión con sus credenciales

  Scenario: Un visitante intenta registrarse con rol ADMIN
    Given el email "hacker@email.com" no está registrado
    When un visitante envía una solicitud para crear un usuario con:
      | nombre    | email             | contraseña | rol  |
      | Hacker    | hacker@email.com  | pass123    | ADMIN |
    Then el sistema crea el usuario con rol CLIENTE ignorando el rol solicitado

  Scenario: Un visitante intenta registrarse con email ya registrado
    Given el email "carlos@email.com" ya está registrado
    When un visitante envía una solicitud para crear un usuario con:
      | nombre     | email            | contraseña |
      | Juan Pérez | carlos@email.com | 1234       |
...
```gherkin
Feature: Gestionar mi perfil

  Background:
    Given un usuario está autenticado con token JWT válido

  Scenario: El usuario consulta su propio perfil
    Given el usuario tiene nombre "Carlos Pérez" y email "carlos@email.com"
    When el usuario solicita ver su perfil
    Then el sistema muestra sus datos
         y su nombre es "Carlos Pérez"
         y su email es "carlos@email.com"
         y su rol es "CLIENTE"

  Scenario: El usuario actualiza su nombre
    Given el usuario tiene nombre "Carlos Pérez"
    When el usuario envía una solicitud para actualizar su perfil con:
      | nombre              | email            | contraseña |
      | Carlos Actualizado  | carlos@email.com | 1234       |
    Then el sistema confirma la actualización
         y su nombre ahora es "Carlos Actualizado"
         y su email permanece "carlos@email.com"
         y su rol no cambia

  Scenario: El usuario actualiza su email
    Given el usuario tiene email "carlos@email.com"
    When el usuario envía una solicitud para actualizar su perfil con:
      | nombre       | email                   | contraseña |
      | Carlos Pérez | carlos_nuevo@email.com  | 1234       |
    Then el sistema confirma la actualización
         y su nuevo email es "carlos_nuevo@email.com"
         y puede iniciar sesión con el nuevo email

  Scenario: El usuario intenta cambiar su propio rol
    Given el usuario tiene rol "CLIENTE"
    When el usuario envía una solicitud para actualizar su perfil incluyendo un campo "role" con valor "ADMIN"
    Then el sistema ignora el campo role
         y su rol permanece "CLIENTE"

  Scenario: El usuario intenta actualizar con email ya registrado por otro
    Given el usuario tiene email "carlos@email.com"
          y otro usuario tiene email "maria@email.com"
    When el usuario intenta cambiar su email a "maria@email.com"
    Then el sistema informa que ese email ya está en uso
         y no se realiza la actualización

  Scenario: El usuario intenta ver su perfil sin autenticarse
    When un usuario no autenticado intenta acceder a su perfil
    Then el sistema rechaza la operación con estado 401

  Scenario: El usuario elimina su propia cuenta
    Given el usuario tiene nombre "Carlos Pérez" y email "carlos@email.com"
    When el usuario solicita eliminar su cuenta
    Then el sistema confirma la eliminación
         y el usuario ya no puede iniciar sesión
         y su email queda libre para nuevo registro
```
---
