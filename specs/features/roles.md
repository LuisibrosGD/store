...
Feature: Endpoints públicos
  ...
  Scenario Outline: Cualquier usuario puede acceder a <endpoint>
    When se envía una solicitud a <endpoint>
    Then la operación es permitida
  ...
  Scenario: El registro público solo permite rol CLIENTE
    When un visitante envía una solicitud para crear un usuario con:
      | nombre     | email             | contraseña | rol  |
      | Nuevo User | nuevo@email.com   | pass123    | ADMIN |
...
- El endpoint `POST /api/auth/refresh` requiere autenticación (token válido) para renovar la sesión.
- El registro público (`POST /api/usuarios`) siempre asigna rol CLIENTE, sin importar el rol solicitado.
