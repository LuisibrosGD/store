...
Feature: Gestión de estados de una orden
  ...
  Scenario: El cliente no puede gestionar estados de órdenes
    Given el cliente tiene una orden pendiente
    When el cliente intenta cambiar el estado de su orden a "PAGADA"
    Then el sistema rechaza la operación con estado 403

  Scenario: El cliente no puede ver órdenes de otros clientes
    Given otro cliente tiene una orden con id 2
    When el cliente solicita ver los datos de la orden 2
    Then el sistema informa que la orden no fue encontrada
```
